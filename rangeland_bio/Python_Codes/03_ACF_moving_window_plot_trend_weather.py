...
# %%
SF_west.drop(columns=["medians_diff_ANPP",
                      "medians_diff_slope_ANPP",
                      "median_ANPP_change_as_perc",
                      "state_majority_area",
                      "p_Spearman",
                      "trend_yue", "p_yue", "trend_rao",
                      "p_rao", "Tau",
                      "state_1", "state_2",
                      "hucsgree_4",
                      "bps_code", "bps_model", "bps_name", "groupveg",
                      "Spearman",
                      "sens_inter", "sens_slope", "trend"],
             inplace=True)
...
# %%
len(list(ACF_trends_MK_df.columns))
...
# ACF_trends_MK_df.rename(columns={col: f"ACF1_{col}" if any(key in col for key in ['slope', 'trend', 'p_value']) 
#                                  else col for col in ACF_trends_MK_df.columns}, 
#                         inplace=True)
...
bad_cols = [col for col in ACF_trends_MK_df.columns if any(key in col for key in ['trend_ws', 'p_value'])]
ACF_trends_MK_df.drop(columns=bad_cols, inplace=True)

# %%
len(ACF_trends_MK_df.columns)
...
weather_variables = [x.replace("_detrendLinReg", '').replace("_detrendSens", "") for x in weather_variables]
...
for a_variable in weather_variables:
    for trendType in ['', '_detrendLinReg','_detrendSens']:
        all_win_sizes_cols = [f'slope_ws{ws}_{a_variable}{trendType}' for ws in range(5, 11)]
        ...
        outdir = bio_plots + acf_or_variance + "_" + a_variable + "/slope/identical_colorbar/"
        ...
        for col in all_win_sizes_cols:
            ...
            cbar1 = fig.colorbar(cent_plt.collections[1], ax=ax, 
                                 orientation='horizontal', shrink=0.3, cax=cax)
...
outdir = bio_plots + acf_or_variance + "_" + a_variable + "/slope/individual_colorbar/"
os.makedirs(outdir, exist_ok=True)
