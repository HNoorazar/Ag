...
for a_pattern in temp_patterns:
    ...
    L_ = f"ACF1-delta & -ratio after & before ANPP-BP1 ({a_pattern})"
    fig.suptitle(L_, y=0.82, fontdict=fontdict_bold_sup);
    
    file_name = curr_out_dir + "ACF_" + a_pattern + "_BP1.png"
...
for y_var in delta_ratio_cols:
    ...
    if "delta" in y_var:
        L_ = r'$\Delta(ACF1_{BP1})$'
        cbar0.set_label(L_, labelpad=1, fontdict=fontdict_normal);
        cbar1.set_label(L_, labelpad=1, fontdict=fontdict_normal);
        pre_title = "diff."
    elif "ratio" in y_var:
        L_ = r'$ratio(ACF1_{BP1})$'
        cbar0.set_label(L_, labelpad=1, fontdict=fontdict_normal);
        cbar1.set_label(L_, labelpad=1, fontdict=fontdict_normal);
        pre_title = "ratio"
    
    t_ = y_var.split("_")[-1]
    title_ = f"ACF1-{pre_title} after & before ANPP-BP1 ({t_})"
    fig.suptitle(title_, y=0.82, fontdict=fontdict_bold_sup);
