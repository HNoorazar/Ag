...
outcsv_file = outcsv_path + "comb_ecozone_gridmet_range.csv"
foutcsv = open(outcsv_file, "w")
foutcsv.write("ecozone,gridmet,count\n")
...
for row in range(0,drows):
  ...
  for col in range(0,dcols):
    ...
    test = Combine([ecozone, gridmet])
    test.save("test")
    #arcpy.management.BuildRasterAttributeTable(test,"Overwrite")
    fields = ['ecol3_90m','gridmet90m','Count']
