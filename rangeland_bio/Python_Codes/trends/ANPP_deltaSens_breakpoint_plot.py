...
sensSlope_beforeAfter_BP1["slope_ratio"] = sensSlope_beforeAfter_BP1["slope_after"] / \
                                                sensSlope_beforeAfter_BP1["slope_before"]
sensSlope_beforeAfter_BP1.head(2)
...
# %%
cols_ = ['fid', 'slope_diff', 'slope_ratio']
...
cbar1.set_label(r'$\Delta(ANPPSs_{BP1})$', labelpad=1, fontdict=fontdict_normal);
plt.title("ANPP slope diff. (ANPP-BP1)", fontdict=fontdict_bold);
...
file_name = breakpoint_plot_base + "ANPP_senSlopeDelta_ANPPBP1_divergeRB_grey.png"
...
cax = ax.inset_axes(inset_axes_)
cbar1 = fig.colorbar(cent_plt.collections[1], ax=ax, orientation='horizontal', shrink=0.3, norm=norm1, cax=cax)
cbar1.set_label(r'$ratio(ANPPSs_{BP1})$', labelpad=1, fontdict=fontdict_normal);
plt.title("ANPP slope ratio (ANPP-BP1)", fontdict=fontdict_bold);
...
# fig.subplots_adjust(top=0.91, bottom=0.01, left=-0.1, right=1)
file_name = breakpoint_plot_base + "ANPP_senSlopeRatio_ANPPBP1_divergeRB_grey.png"
plt.savefig(file_name, bbox_inches='tight', dpi=map_dpi_)
