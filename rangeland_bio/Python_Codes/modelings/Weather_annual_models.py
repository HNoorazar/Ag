...
# %%
# changed Dec 9, 2024 to drop 2012: Need to run
...
monthly_weather = monthly_weather[
    monthly_weather["fid"].isin(list(FIDs_weather_ANPP_common["fid"]))
]
...
annual_WA_ANPP = pd.merge(
    annual_WA_ANPP, annual_rel_hum, how="left", on=["fid", "year"]
)

annual_WA_ANPP = pd.merge(
    annual_WA_ANPP,
    ANPP[["fid", "year", "mean_lb_per_acr"]],
    how="left",
    on=["fid", "year"],
)

annual_WA_ANPP.rename(
    columns={
        "precip_mm_month": "precip_mm_yr",
...
# %%
annual_WA_ANPP = pd.merge(
    annual_WA_ANPP, SF_west[["fid", "groupveg"]], on="fid", how="left"
)
annual_WA_ANPP.head(2)

# Lets just forget abuot Sparse, Riparian, Barren-Rock/Sand/Clay, and Conifer?
annual_WA_ANPP[["groupveg", "fid"]].drop_duplicates().groupby(
    ["groupveg"]
).count().reset_index()
...
params = {
    "legend.fontsize": tick_legend_FontSize * 0.8,
    "axes.labelsize": tick_legend_FontSize * 0.8,
...
fig, axes = plt.subplots(
    ...
    sharey=True,
    sharex=True,
    gridspec_kw={"hspace": 0.25, "wspace": 0.01},
    dpi=dpi_,
)
sns.set_style({"axes.grid": True})
...
sns.histplot(
    data=df["mean_lb_per_acr"],
    ax=axes[0][0],
    bins=100,
    kde=True,
    color=veg_colors[veg_type],
)
...
axes[0][0].set_xlabel("")
axes[0][1].set_xlabel("")
axes[1][0].set_xlabel("ANPP (mean lb/acr)")
axes[1][1].set_xlabel("ANPP (mean lb/acr)")

fig.suptitle("ANPP distribution", y=0.93, fontsize=18)
fig.subplots_adjust(
    top=0.85, bottom=0.15, left=0.052, right=0.981, wspace=-0.2, hspace=0
)
...
params = {
    "legend.fontsize": tick_legend_FontSize * 0.6,
...
plt.rcParams["axes.linewidth"] = 0.051
plt.tight_layout()
...
# %%
min_color = min(
    SF_west_Spearman_95["temp_Spearman"].min(),
    SF_west_Spearman_95["precip_Spearman"].min(),
)
max_color = max(
    SF_west_Spearman_95["temp_Spearman"].max(),
    SF_west_Spearman_95["precip_Spearman"].max(),
)
...
####################################################################################
p1 = SF_west_Spearman_95.plot(
    column="temp_Spearman", ax=ax1, cmap=cmap_RYG, norm=norm_colorB, legend=False
)
ax1.set_title("temperature (Spearman's rank; 95% significant)")

p2 = SF_west_Spearman_95.plot(
    column="precip_Spearman", ax=ax2, cmap=cmap_RYG, norm=norm_colorB, legend=False
)
...
fig.subplots_adjust(
    top=0.91, bottom=0.08, left=0.082, right=0.981, wspace=-0.2, hspace=0
)
fig.colorbar(
    p1.get_children()[1],
    ax=ax2,
...
# %%
SF_west_Spearman_95_green = SF_west_Spearman_95[
    SF_west_Spearman_95["trend"] == "increasing"
].copy()
...
# %%
min_color = min(
    SF_west_Spearman_95_green["temp_Spearman"].min(),
    SF_west_Spearman_95_green["precip_Spearman"].min(),
)
max_color = max(
    SF_west_Spearman_95_green["temp_Spearman"].max(),
...
plot_SF(SF=visframe_mainLand_west, cmap_="Pastel1", ax_=ax2, col="EW_meridian")
####################################################################################
p1 = SF_west_Spearman_95_green.plot(
    column="temp_Spearman", ax=ax1, cmap=cmap_RYG, norm=norm_colorB
)
ax1.set_title("temperature (Spearman's rank; 95% significant)")

p2 = SF_west_Spearman_95_green.plot(
    column="precip_Spearman", ax=ax2, cmap=cmap_RYG, norm=norm_colorB
)
ax2.set_title("precipitation (Spearman's rank; 95% significant)")
####################################################################################
...
fig.subplots_adjust(
    top=0.91, bottom=0.08, left=0.082, right=0.981, wspace=-0.1, hspace=0
)
...
prec_sprear = round(
    temp_precip_spear[temp_precip_spear["fid"] == a_fid]["precip_Spearman"].values[0], 2
)
temp_sprear = round(
    temp_precip_spear[temp_precip_spear["fid"] == a_fid]["temp_Spearman"].values[0], 2
)
...
text_1 = "trend: {}\nSen's slope {}, reg. slope {}\n{}".format(
    trend_, slope_, reg_slope, state_
)
text_2 = " (FID: {})\nPrecip. Spearman {}\nTemp. Spearman {}".format(
    a_fid, prec_sprear, temp_sprear
)
text_ = text_1 + text_2
...
tick_legend_FontSize = 12
params = {
    "legend.fontsize": tick_legend_FontSize * 0.81,
...
fig, axes = plt.subplots(
    ...
    figsize=(10, 6),
    sharey=False,
    gridspec_kw={"hspace": 0.5, "wspace": 0.03},
    dpi=dpi_,
)
...
title_ = pre_title.format(state_, a_fid, prec_sprear, temp_sprear)
ax5.set_title(title_)
ax6.set_title(title_)
#####################################################################################
...
# %%
annual_WA_ANPP["temp_X_precip"] = (
    annual_WA_ANPP["precip_cm_yr"]
...
my_scatter = sns.pairplot(
    A[cols_], size=1.5, diag_kind="None", plot_kws={"s": 4}, corner=True
)
...
m5 = spreg.OLS_Regimes(
    y=annual_WA_ANPP[depen_var].values,
...
m5_results = pd.DataFrame(
    {
        "Coeff.": m5.betas.flatten(),  # Pull out regression coefficients and
...
X_train, X_test, y_train, y_test = train_test_split(
    indp_df, y_df, test_size=0.3, random_state=42
)
X_train.head(2)
...
m5_solo = spreg.OLS(
    y=y_tr_.values, x=X_tr_[indp_vars].values, name_y=depen_var, name_x=indp_vars
)

print(f"{m5_solo.r2.round(2) = }")

m5_solo_results = pd.DataFrame(
    {
        "Coeff.": m5_solo.betas.flatten(),
...
ks_result = ks.fit()
y_pred = ks_result.predict(
    X[["const", "precip_cm_yr", "avg_of_dailyAvgTemp_C_AvgOverMonths"]]
)
ks_result.params
...
m5_T_normal_results = pd.DataFrame(
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_T_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_T_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
m5_P_normal_results = pd.DataFrame(
    {
        "Coeff.": m5_P_normal.betas.flatten(),
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_P_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_P_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
m5_TP_normal = spreg.OLS_Regimes(
    y=y_train.values,
    x=X_train_normal[indp_vars].values,
    regimes=X_train_normal["groupveg"].tolist(),
    constant_regi="many",
...
m5_TP_normal_results = pd.DataFrame(
    {
        "Coeff.": m5_TP_normal.betas.flatten(),
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_TP_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_TP_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
m5_TPinter_normal = spreg.OLS_Regimes(
    y=y_train.values,
    x=X_train_normal[indp_vars].values,
    regimes=X_train_normal["groupveg"].tolist(),
    constant_regi="many",
...
m5_TPinter_normal_results = pd.DataFrame(
    {
        "Coeff.": m5_TPinter_normal.betas.flatten(),
...
veg_ = "Conifer"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Conifer = m5_TPinter_normal_results.loc[Conifer_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Conifer.columns = pd.MultiIndex.from_product([[veg_], Conifer.columns])
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_TPinter_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Hardwood"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Hardwood = m5_TPinter_normal_results.loc[Hardwood_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Hardwood.columns = pd.MultiIndex.from_product([[veg_], Hardwood.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_TPinter_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
m5_TPH_normal = spreg.OLS_Regimes(
    y=y_train.values,
    x=X_train_normal[indp_vars].values,
    regimes=X_train_normal["groupveg"].tolist(),
    constant_regi="many",
...
m5_TPH_normal_results = pd.DataFrame(
    {
        "Coeff.": m5_TPH_normal.betas.flatten(),
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_TPH_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Hardwood"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Hardwood = m5_TPH_normal_results.loc[Hardwood_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Hardwood.columns = pd.MultiIndex.from_product([[veg_], Hardwood.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_TPH_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
m5_TP_sq_normal = spreg.OLS_Regimes(
    y=y_train.values,
    x=X_train_normal[indp_vars].values,
    regimes=X_train_normal["groupveg"].tolist(),
    constant_regi="many",
...
m5_TP_sq_normal_results = pd.DataFrame(
    {
        "Coeff.": m5_TP_sq_normal.betas.flatten(),
...
veg_ = "Conifer"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Conifer = m5_TP_sq_normal_results.loc[Conifer_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Conifer.columns = pd.MultiIndex.from_product([[veg_], Conifer.columns])
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_TP_sq_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Hardwood"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Hardwood = m5_TP_sq_normal_results.loc[Hardwood_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Hardwood.columns = pd.MultiIndex.from_product([[veg_], Hardwood.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_TP_sq_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
m5_TP_normal_logy = spreg.OLS_Regimes(
    y=np.log10(y_train.values),
    x=X_train_normal[indp_vars].values,
    regimes=X_train_normal["groupveg"].tolist(),
    constant_regi="many",
    regime_err_sep=False,
...
m5_TP_normal_logy_results = pd.DataFrame(
    {
        "Coeff.": m5_TP_normal_logy.betas.flatten(),
...
veg_ = "Conifer"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Conifer = m5_TP_normal_logy_results.loc[Conifer_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Conifer.columns = pd.MultiIndex.from_product([[veg_], Conifer.columns])
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_TP_normal_logy_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Hardwood"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Hardwood = m5_TP_normal_logy_results.loc[Hardwood_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Hardwood.columns = pd.MultiIndex.from_product([[veg_], Hardwood.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_TP_normal_logy_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
...
axes.scatter(
    X_train["precip_cm_yr"],
    X_train["avg_of_dailyAvgTemp_C_AvgOverMonths"],
    c="dodgerblue",
    s=2,
)
...
m5_TP_cube_normal = spreg.OLS_Regimes(
    y=y_train.values,
    x=X_train_normal[indp_vars].values,
    regimes=X_train_normal["groupveg"].tolist(),
    constant_regi="many",
...
m5_TP_cube_normal_results = pd.DataFrame(
    {
        "Coeff.": m5_TP_cube_normal.betas.flatten(),
...
veg_ = "Conifer"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Conifer = m5_TP_cube_normal_results.loc[Conifer_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Conifer.columns = pd.MultiIndex.from_product([[veg_], Conifer.columns])
...
veg_ = "Grassland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Grassland = m5_TP_cube_normal_results.loc[Grassland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Grassland.columns = pd.MultiIndex.from_product([[veg_], Grassland.columns])
...
veg_ = "Hardwood"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Hardwood = m5_TP_cube_normal_results.loc[Hardwood_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Hardwood.columns = pd.MultiIndex.from_product([[veg_], Hardwood.columns])
...
veg_ = "Shrubland"
rep_ = [x for x in groupveg if veg_ in x][0] + "_"
Shrubland = m5_TP_cube_normal_results.loc[Shrubland_m, :].rename(
    lambda i: i.replace(rep_, "")
)
Shrubland.columns = pd.MultiIndex.from_product([[veg_], Shrubland.columns])
