...
new_cols_order = first_cols + [col for col in bps_weather.columns if col not in first_cols]
bps_weather = bps_weather[new_cols_order]
bps_weather.head(2)

# %%
list(bps_weather.columns)[5:]
...
# %%
temp_list = [x for x in sorted(grouped_stats.columns) if 'Temp_C' in x]
temp_list = [x for x in temp_list if (not ('max' in x))]
temp_list = [x for x in temp_list if (not ('detrend' in x))]
temp_list = [x for x in temp_list if (not ('yearcv' in x))]
temp_list = [x for x in temp_list if (not ('40yearvar' in x))]
temp_list
...
# %%
num_locs = len(bps_weather["fid"].unique())
num_locs
...
# %%
variable_ = "40yearvar_avg_of_dailyAvgTemp_C"
print (Albers_SF[variable_].max())
print (Albers_SF[variable_].min())

# %%
variable_ = "40yearcv_avg_of_dailyAvgTemp_C_times100"

# %%
print (Albers_SF[f"{variable_}"].max())
print (Albers_SF[f"{variable_}"].min())
...
# %%
cv_columns = [x for x in grouped_stats.columns if "40yearcv_" in x]
len(cv_columns)

# %%
for variable_ in list(grouped_stats.columns)[1:]: # cv_columns: # lets do all.
...
# %%
y_var = "40yearmin_avg_of_dailyAvgTemp_C"
print (round(Albers_SF[y_var].max(), 2))
print (round(Albers_SF[y_var].min(), 2))
...
# %%
counter = 1
for y_var in list(grouped_stats.columnsgrouped_stats.columns1)[1:]:
    print (f"{counter} out of {len(list(grouped_stats.columns)[1:])}")
...
ax2.set_title(fr"between {a_percent} and {100-a_percent}".format(cut_1, cut_2), fontdict=fontdict_bold);
######################################################
...
# %%
for y_var in cv_columns:
    for a_percent in [5, 10]:
        perc_ = a_percent / 100
        lower_bound = Albers_SF[y_var].quantile(perc_)
        upper_bound = Albers_SF[y_var].quantile(1 - perc_)

        # Filter rows between and outside outlier boundaries
        lower_df = Albers_SF[Albers_SF[y_var] < lower_bound]
        between_df = Albers_SF[(Albers_SF[y_var] >= lower_bound) & (Albers_SF[y_var] <= upper_bound)]
        upper_df = Albers_SF[Albers_SF[y_var] <= upper_bound]        
        ...
        cbar1.ax.tick_params(labelsize=font_base*font_c)
        cbar1.set_label(cbar_label, labelpad=1, fontdict=fontdict_normal, fontsize=font_base*font_c);
        ax1.set_title(f"lower {a_percent}%", fontdict=fontdict_bold);
        ######################################################
        ...
        cbar2.set_label(cbar_label, labelpad=1, fontdict=fontdict_normal,fontsize=font_base*font_c);
        # ax2.set_title(f"CV(temp) in [{cut_1}, {cut_2}]", fontdict=fontdict_bold);
        ax2.set_title(r"between {a_percent} and {100-a_percent}".format(cut_1, cut_2), fontdict=fontdict_bold);
        ######################################################
        cax = ax3.inset_axes(inset_axes_)
        cbar3 = fig.colorbar(cent_plt2.collections[1], ax=ax3, orientation='horizontal', shrink=0.3, cax=cax)
        cbar3.ax.tick_params(labelsize=font_base*font_c)
        cbar3.set_label(cbar_label, labelpad=1, fontdict=fontdict_normal, fontsize=font_base*font_c);
        ax3.set_title(f"upper {a_percent}%", fontdict=fontdict_bold);
