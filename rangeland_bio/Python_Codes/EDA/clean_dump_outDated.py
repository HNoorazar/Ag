...
bps_weather.rename(
    columns={
        "rmin_min": "min_of_dailyMin_rel_hum",
...
bpszone_ANPP.rename(
    columns={"area": "area_sqMeter", "count": "pixel_count", "mean": "mean_lb_per_acr"},
    inplace=True,
)
...
FIDs_weather_ANPP_common = set(bps_weather["fid"].unique()).intersection(
    set(bpszone_ANPP["fid"].unique())
)
FIDs_weather_ANPP_common = list(
    set(FIDs_weather_ANPP_common).intersection(Albers_SF_west["fid"].unique())
)
FIDs_weather_ANPP_common = pd.DataFrame(columns=["fid"], data=FIDs_weather_ANPP_common)

FIDs_weather_ANPP_common = pd.merge(
    FIDs_weather_ANPP_common,
...
bpszone_ANPP_west = bpszone_ANPP[
    bpszone_ANPP["fid"].isin(list(Albers_SF_west["fid"]))
].copy()
...
plot_SF(SF=visframe_mainLand, ax_=ax, col="EW_meridian", cmap_="Pastel1")
...
# %%
median_diff = bpszone_ANPP_west[
    ["fid", "state_majority_area", "state_1", "state_2", "EW_meridian"]
].copy()
...
for a_FID in median_diff["fid"].unique():
    ...
    median_diff.loc[
        median_diff["fid"] == a_FID, "first_10_years_median_ANPP"
    ] = first_decade["mean_lb_per_acr"].median()

    median_diff.loc[
        median_diff["fid"] == a_FID, "last_10_years_median_ANPP"
...
median_diff["medians_diff_ANPP"] = (
    median_diff["last_10_years_median_ANPP"] - median_diff["first_10_years_median_ANPP"]
)
...
# %%
median_diff["median_ANPP_change_as_perc"] = (
    100 * median_diff["medians_diff_ANPP"]
...
ANPP_TS = bpszone_ANPP_west.loc[
    bpszone_ANPP_west.fid == a_FID, "mean_lb_per_acr"
].values
...
for a_FID in bpszone_ANPP_west["fid"].unique():
    ...
    if (
        (intercept != intercept_rao)
        or (intercept != intercept_yue)
        or (intercept_rao != intercept_yue)
    ):
...
for a_FID in ANPP_MK_df["fid"].unique():
    ...
    trend_yue, _, p_yue, _, _, _, var_s_yue, _, _ = mk.yue_wang_modification_test(
        ANPP_TS
    )
    trend_rao, _, p_rao, _, _, _, var_s_rao, _, _ = mk.hamed_rao_modification_test(
        ANPP_TS
    )
...
trend_col = "trend"
trend_count_orig = (
    ANPP_MK_df[[trend_col, "fid"]].groupby([trend_col]).count().reset_index()
)
...
trend_count_yue.rename(columns={"fid": "fid_yue", "trend_yue": "trend"}, inplace=True)
trend_count_yue
...
trend_count_rao.rename(columns={"fid": "fid_rao", "trend_rao": "trend"}, inplace=True)
trend_count_rao
...
spearman_increase_pval5 = spearman_increase_pval5[
    spearman_increase_pval5["Spearman"] > 0
]
spearman_increase_pval5.shape

# %%
spearman_decrease_pval5 = ANPP_MK_df[ANPP_MK_df["p_Spearman"] < 0.05]
spearman_decrease_pval5 = spearman_decrease_pval5[
    spearman_decrease_pval5["Spearman"] < 0
]
...
decreasing_df_orig = (
    decreasing_df_orig[["fid", "state_majority_area"]]
    .groupby(["state_majority_area"])
    .count()
    .reset_index()
)
...
decreasing_df_yue = (
    decreasing_df_yue[["fid", "state_majority_area"]]
    .groupby(["state_majority_area"])
    .count()
    .reset_index()
)
...
decreasing_df_rao = (
    decreasing_df_rao[["fid", "state_majority_area"]]
    .groupby(["state_majority_area"])
    .count()
    .reset_index()
)
...
# .zip does not work for some versions of library!!!
f_name = bio_reOrganized + "Albers_SF_west_ANPP_MK_Spearman.shp"
