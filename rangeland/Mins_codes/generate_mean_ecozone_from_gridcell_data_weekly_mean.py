        ...
        if 'gridmet' not in a:
            ...
            if True:
                ...
                if gridmet in gridmet_info:
                    ...
                    if Path(filename).is_file():
                        ...
                        gridmet_indexes = pd.read_csv(filename,header=0,dtype={'year': int,'Week_Number': int})
                        #print('read done!')
            ...
            if county not in county_indices and not gridmet_indexes.empty:
                ...
                for col in county_indices[county].columns:
                    if col not in ['year','Week_Number','total_fraction']:
...
for county in county_indices:
    ...
    county_indices[county]['ecozone'] = county
...
#export
#float_fmt = lambda x: '%.3f' % x if x.name not in ['year','Week_Number','county'] else x
#all_county_indices.to_csv(out_climate_indicex,index=False,float_format=float_fmt)

all_county_indices[['year','Week_Number']] = all_county_indices[['year','Week_Number']].astype(int)
