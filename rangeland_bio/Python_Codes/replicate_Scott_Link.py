...
# Define the Albers Equal-Area projection using the PROJ string
proj_str = "+proj=aea +lat_1=34 +lat_2=40.5 +lat_0=0 +lon_0=-120 +x_0=0 +y_0=-4000000 +datum=WGS84 +units=m"
crs_alb = CRS.from_proj4(proj_str)

# Example: Transform coordinates from WGS84 (lat/lon) to Albers Equal-Area (meters)
transformer = Transformer.from_crs("EPSG:4326", crs_alb, always_xy=True)
...
# Reverse transformation (Albers to WGS84)
lon_rev, lat_rev = transformer.transform(x, y, direction="INVERSE")
print(f"Original coordinates: Longitude={lon_rev}, Latitude={lat_rev}")
...
# %%
albers_long, albers_lat = transformer.transform(gdf["LONG"], gdf["LAT"])

# %%
gdf["albers_long"] = albers_long
gdf["albers_lat"] = albers_lat
gdf.head(2)
...
y = gdf["pan"].values.reshape(-1, 1)  # Dependent variable
X = gdf[["ALT"]].values  # Independent variables
...
# Fit GWR model using optimal bandwidth
...
# Select bandwidth
# https://mgwr.readthedocs.io/en/latest/generated/mgwr.sel_bw.Sel_BW.html
selector = Sel_BW(coords, y, X, kernel="bisquare", family=Gaussian(), fixed=False, constant=False, spherical=False)
optimal_bandwidth = selector.search(criterion='CV')
print(f"Optimal Bandwidth: {optimal_bandwidth}")
